...
(function(HUB) {
  ...
  var Collapsible = (MathJax.Extension.collapsible = {
    ...
    //    Marker: function (c) {return MML.mtext("\u25B9"+c+"\u25C3").With({mathcolor:"blue",attr:{},attrNames:[]})},
    Marker: function(c) {
      return MML.mtext("\u25C2" + c + "\u25B8").With({ mathcolor: "blue", attr: {}, attrNames: [] });
    },
    //    Marker: function (c) {return MML.mtext("\u25B8"+c+"\u25C2").With({mathcolor:"blue",attr:{},attrNames:[]})},
    //    Marker: function (c) {return MML.mtext("\u27EA"+c+"\u27EB").With({mathcolor:"blue",attr:{},attrNames:[]})},
    ...
    Collapse_sqrt: function(mml) {
      ...
      if (mml.complexity > this.COLLAPSE.sqrt) mml = this.MakeAction(this.Marker(this.MARKER.sqrt), mml);
      return mml;
    },
    ...
    Collapse_superscript: function(mml) {
      ...
      if (mml.complexity > this.COLLAPSE.superscript) mml = this.MakeAction(this.Marker(this.MARKER.superscript), mml);
      return mml;
    },
    Collapse_subscript: function(mml) {
      this.UncollapseChild(mml, 0, 2);
      if (mml.complexity > this.COLLAPSE.subscript) mml = this.MakeAction(this.Marker(this.MARKER.subscript), mml);
      return mml;
    },
    Collapse_subsup: function(mml) {
      this.UncollapseChild(mml, 0, 3);
      if (mml.complexity > this.COLLAPSE.subsup) mml = this.MakeAction(this.Marker(this.MARKER.subsup), mml);
      return mml;
    }
  });
...
MathJax.Hub.Register.StartupHook("Semantic Enrich Ready", function() {
  ...
  MML.mbase.Augment({
    ...
    //
    Collapse: function() {
      return Collapsible.Collapse(this);
    },
    //
