...
MathJax.Hub.Register.StartupHook("HTML-CSS Jax Ready", function() {
  ...
  MML.mbase.Augment({
    ...
    HTMLmultiline: function(span) {
      ...
      //
      var parent = this;
      while (parent.inferred || (parent.parent && parent.parent.type === "mrow" && parent.isEmbellished())) {
      ...
      while (
        this.HTMLbetterBreak(end, state, true) &&
    ...
    HTMLbetterBreak: function(info, state, toplevel) {
      ...
      if (toplevel && better) {
        ...
        if (MO.HTMLbetterBreak(info, state)) {
          better = false;
          index = info.index;
        }
      }
