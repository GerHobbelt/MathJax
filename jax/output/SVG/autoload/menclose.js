...
MathJax.Hub.Register.StartupHook("SVG Jax Ready", function() {
  ...
  MML.menclose.Augment({
    toSVG: function(HW, DD) {
      ...
      var nl = MathJax.Hub.SplitList(values.notation),
        notation = {};
      ...
      for (var n in notation) {
        ...
        switch (n) {
          ...
          case MML.NOTATION.ROUNDEDBOX:
            svg.Add(
              BBOX.FRAME(H, D, W, t, "solid", values.mathcolor, { rx: Math.floor(Math.min(H + D - t, W - t) / 4) })
            );
            break;
          ...
          case MML.NOTATION.UPDIAGONALARROW:
            ...
            h = (H + D) * f;
            var x = 0.4 * h;
            svg.Add(BBOX.DLINE(H - 0.5 * h, D, W - 0.5 * w, t, values.mathcolor, "up"));
            svg.Add(
              BBOX.FPOLY(
                [[x + w, h], [x - 0.4 * h, 0.4 * w], [x + 0.3 * w, 0.3 * h], [x + 0.4 * h, -0.4 * w], [x + w, h]],
                values.mathcolor
              ),
              W - w - x,
              H - h
          ...
          case MML.NOTATION.RADICAL:
            svg.Add(
              BBOX.PPATH(
                ...
                W,
                "M " +
                  this.SVGxy(t / 2, 0.4 * (H + D)) +
                  " L " +
                  this.SVGxy(p, t / 2) +
                  " L " +
                  this.SVGxy(2 * p, H + D - t / 2) +
                  " L " +
                  this.SVGxy(W, H + D - t / 2),
                t,
                values.mathcolor
              ),
              0,
            ...
            svg.Add(
              BBOX.PPATH(
                ...
                "M " +
                  this.SVGxy(t / 2, t / 2) +
                  " a " +
                  this.SVGxy(p, (H + D) / 2 - 2 * t) +
                  " 0 0,1 " +
                  this.SVGxy(t / 2, H + D - t) +
                  " L " +
                  this.SVGxy(W, H + D - t / 2),
                t,
                values.mathcolor
