...
MathJax.Hub.Register.StartupHook("CommonHTML Jax Ready", function() {
  ...
  MML.mbase.Augment({
    ...
    CHTMLmultiline: function(node) {
      ...
      //
      var parent = this;
      while (parent.inferred || (parent.parent && parent.parent.type === "mrow" && parent.parent.isEmbellished())) {
      ...
      while (
        this.CHTMLbetterBreak(end, state, true) &&
    ...
    CHTMLbetterBreak: function(info, state, toplevel) {
      ...
      if (toplevel && better) {
        ...
        if (MO.CHTMLbetterBreak(info, state)) {
          better = false;
          index = info.index;
        }
      }
  ...
  MML.mo.Augment({
    ...
    CHTMLbetterBreak: function(info, state) {
      ...
      if (W - info.shift === 0 && values.linebreak !== MML.LINEBREAK.NEWLINE) return false; // don't break at zero width (FIXME?)
      ...
      if (
        (values.linebreakstyle === MML.LINEBREAKSTYLE.AFTER && values.texClass === MML.TEXCLASS.OPEN) ||
