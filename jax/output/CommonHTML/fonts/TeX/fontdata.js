...
(function(CHTML, MML, AJAX) {
  ...
  CHTML.FONTDEF.TeX = {
    ...
    VARIANT: {
      ...
      italic: {
        fonts: [ITALIC, "MathJax_Main-Italic"],
        italic: true,
        cache: {},
        chain: "normal",
        remap: {
      ...
      "-TeX-variant": {
        fonts: [AMS, MAIN, SIZE1],
        cache: {}, // HACK: to get larger prime for \prime
        remap: {
          0x2268: 0xe00c,
          0x2269: 0xe00d,
          0x2270: 0xe011,
    ...
    REMAPACCENT: {
      ...
      "\u2035": "`",
      "\u20D0": "\u21BC",
      "\u20D1": "\u21C0", // combining left and right harpoons
      "\u20D6": "\u2190",
      "\u20E1": "\u2194", // combining left arrow and lef-right arrow
    ...
    REMAPACCENTUNDER: {
      "\u20EC": "\u21C1",
      "\u20ED": "\u21BD", // combining low right and left harpoons
      "\u20EE": "\u2190",
      "\u20EF": "\u2192", // combining low left and right arrows
      "\u20DB": "...", // combining three dots above
      "\u20DC": "...." // combining four dots above
    },
    ...
    DELIMITERS: {
      ...
      0x02c6: {
        dir: H,
        HW: [[0.267 + 0.25, MAIN], [0.567 + 0.25, SIZE1], [1.005 + 0.33, SIZE2], [1.447 + 0.33, SIZE3], [1.909, SIZE4]]
      },
      // wide tilde
      0x02dc: {
        dir: H,
        HW: [[0.333 + 0.25, MAIN], [0.555 + 0.25, SIZE1], [1 + 0.33, SIZE2], [1.443 + 0.33, SIZE3], [1.887, SIZE4]]
      },
      // en-dash
      0x2013: {
        dir: H,
  ...
  CHTML.FONTDATA.familyName = function(font) {
    ...
    var suffix =
      names[0].toLowerCase().replace(/(?:igraphic|serif|writer|tur|tor)$/, "") + "-" + names[1].replace(/[^A-Z]/g, "");
    return "MJXc-TeX-" + suffix;
  };
  (function() {
    var STYLES = CHTML.config.styles,
      FONTS = CHTML.FONTDATA.FONTS;
    var faces = [];
    for (var name in FONTS) {
      if (FONTS.hasOwnProperty(name)) {
        var family = CHTML.FONTDATA.familyName(name),
          FAMILY = family;
