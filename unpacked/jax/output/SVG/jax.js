...
(function (AJAX,HUB,HTML,SVG) {
  ...
  SVG.Augment({
    ...
    createChar: function (scale,data,font) {
      var text = "";
      var variant = {fonts: [data[1]], noRemap: true};
    ...
    extendDelimiterH: function (svg,W,delim,scale,font) {
      ...
      if (W > w) {
        ...
        var fuzz = delim.fuzz || 0;
        var k = (delim.mid ? 2 : 1);
        var rW = (W-w) / k;
        var s = (rW+fuzz) / (rep.r-rep.l);
        while (k-- > 0) {
  ...
  BBOX.HLINE = BBOX.Subclass({
    ...
    Init: function (w,t,dash,color,def) {
      ...
      if (dash === "dashed") {
        ...
        var m = Math.floor(Math.max(0,w-t)/(2*n+1));
  ...
  BBOX.VLINE = BBOX.Subclass({
    ...
    Init: function (h,t,dash,color,def) {
      ...
      if (dash === "dashed") {
        ...
        var m = Math.floor(Math.max(0,h-t)/(2*n+1));
        def["stroke-dasharray"] = m+" "+m;
      }
      if (dash === "dotted") {
        def["stroke-dasharray"] = [1, Math.max(150, Math.floor(2 * t))].join(" ");
  ...
  HUB.Register.StartupHook("mml Jax Ready",function () {
    ...
    MML.mbase.Augment({
      ...
      SVGaddAttributes: function (svg) {
        ...
        if (this.attrNames) {
          ...
          var skip = MML.nocopyAttributes;
          var ignore = HUB.config.ignoreMMLattributes;
          var defaults = (this.type === "mstyle" ? MML.math.prototype.defaults : this.defaults);
          for (var i = 0, m = copy.length; i < m; i++) {
            var id = copy[i];
            if (
              ignore[id] == false ||
              (!skip[id] && !ignore[id] && defaults[id] == null && typeof svg.element[id] === "undefined")
            ) {
      ...
      SVGhandleColor: function (svg) {
        ...
        var padding = (this.styles||{}).padding;
        var bleft = ((borders||{}).left||0);
        var pleft = ((padding||{}).left||0);
        var id;
    ...
    MML.merror.Augment({
      toSVG: function (HW,D) {
        this.SVGgetStyles();
        var svg = this.SVG();
        var scale = SVG.length2em(this.styles.fontSize || 1) / 1000;
        this.SVGhandleSpace(svg);
    ...
    MML.mrow.Augment({
      ...
      SVGlineBreaks: function (svg) {
        ...
        return (SVG.config.linebreaks.automatic && svg.w > SVG.linebreakWidth) || 
          this.hasNewline();
      },
    ...
    MML.mroot.Augment({
      ...
      SVGrootHeight: function (d,scale,root) {
        return 0.45 * (d - 900 * scale) + 600 * scale + Math.max(0, root.d - 75);
      }
    });
    ...
    MML.msubsup.Augment({
      toSVG: function (HW,D) {
        ...
        var s = SVG.TeX.scriptspace * scale;
        var sup, sub;
        ...
        var r = SVG.TeX.sub_drop * sscale;
        var u = base.h+(base.y||0) - q;
        var v = base.d-(base.y||0) + r;
        var delta = 0;
        var p;
        if (base.ic) {
          base.w -= base.ic;             // remove IC (added by mo and mi)
          delta = 1.3 * base.ic + 0.05;  // adjust faked IC to be more in line with expected results
        }
    ...
    MML.math.Augment({
      ...
      toSVG: function (span,div) {
        ...
        if (this.data[0]) {
          ...
          var r = Math.max(svg.r-svg.w,0);
          var style = svg.element.style;
          var px = SVG.TeX.x_height/SVG.ex;

          // round to pixels and add padding
          var H = (Math.ceil(svg.H/px)+1)*px+SVG.HFUZZ;
          var D = (Math.ceil(svg.D/px)+1)*px+SVG.DFUZZ;
